...
#from pre_train.comp_autoencoder_modeling import *
...
def get_func_repr_task(input_q, output_q):
    ...
    for function_name in tqdm(function_name_list):
        ...
        for schedule_index in range(len(programs_dict[function_name]['schedules_list'])):
            ...
            comps_tensor.apply_(thresholding_bound)
...
class Dataset_parallel:
    def __init__(
        ...
        autoencoder.load_state_dict(torch.load("/scratch/cl5503/cost_model_auto_encoder/pre_train/saved_models/comps_autoencoder.pt", map_location = cuda_device))
        autoencoder.eval()
        self.encoder = autoencoder.encoder.to(cuda_device)
        self.encoder.eval()
        """
        ...
        comp_tensor_placeholder = []
        code_placeholder = []
        for function_name, nb_dropped, tree_footprint, local_function_dict in tqdm(processes_output_list):
            ...
            """
            for comp_tensor in local_function_dict['comps_tensor_list']:
                original_device = comp_tensor.device
                comp_tensor_placeholder = comp_tensor.to(cuda_device)
                with torch.no_grad():
                    code_placeholder = self.encoder(comp_tensor_placeholder).to(original_device)
                batches_dict[tree_footprint]['comps_tensor_list'].append(code_placeholder)
            """
        ...
        for tree_footprint in tqdm(batches_dict):
            ...
            for chunk in range( 0, len(batches_dict[tree_footprint]["speedups_list"]), max_batch_size, ):
                ...
                if storing_device.type == "cuda" and ( torch.cuda.memory_allocated(storing_device.index) / torch.cuda.get_device_properties(storing_device.index).total_memory )> 0.70:
