...
class Model_Recursive_LSTM_v2(nn.Module):
    ...
    def forward(self, tree_tensors):
        ...
        accesses_matrices = third_part[:, 110:].view((-1, 16, 44))

        if self.training and self.is_fine_tune:
            access_embed, (h_n, c_n) = self.encoder(accesses_matrices)
            access_embed = torch.flatten(access_embed, start_dim=1)
        else:
            access_embed, (h_n, c_n) = self.encoder.eval()(accesses_matrices)
            access_embed = torch.flatten(access_embed, start_dim=1)
        
        x = torch.cat((first_part, vectors, third_part[:, :110],access_embed, expr_tree), dim=1).view(batch_size, num_comps, -1)
