...
np_data_train = np.load("pre-train_data/perm100_sequence_dataset_train.npy").astype(np.float32)
np_data_val = np.load("pre-train_data/perm100_sequence_dataset_val.npy").astype(np.float32)
...
for epoch in range(num_epochs):
    ...
    if best_valid_loss > valid_loss:
        ...
        # Saving State Dict
        torch.save(autoencoder.state_dict(), "saved_models/lstm_autoencoder_perm100_005drop.pt")
        #torch.save(autoencoder.encoder.state_dict(), "saved_models/lstm_autoencoder_encoder_perm_2layer.pt")
        #torch.save(autoencoder.decoder.state_dict(), "saved_models/lstm_autoencoder_decoder_perm_2layer.pt") 
...
plt.savefig('plots/lstm_autoencoder_perm100_2layer_005drop.png')
